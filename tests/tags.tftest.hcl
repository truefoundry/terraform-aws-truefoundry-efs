...
run "tags_applied" {
  ...
  assert {
    condition     = aws_iam_policy.efs[0].tags["terraform-module"] == "efs"
    error_message = "Expected terraform-module=efs on aws_iam_policy.efs, got: ${aws_iam_policy.efs[0].tags["terraform-module"]}"
  }
}
